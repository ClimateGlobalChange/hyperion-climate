...
int MeshUtilitiesFuzzy::FindNodeEdgeSide(
	...
	if (edgetype == Edge::Type_GreatCircleArc) {
		double dDotNorm = DotProduct(CrossProduct(nodeBegin, nodeEnd), nodeTest);
		...
		double dAlignment = (nodeBegin.x * nodeEnd.y - nodeEnd.x * nodeBegin.y);
		double dDotNorm =
...
bool MeshUtilitiesFuzzy::CalculateEdgeIntersectionsSemiClip(
	...
	if ((typeFirst  == Edge::Type_GreatCircleArc) &&
		...
		double dDot11 = DotProduct(nodeN21xN22, node11);
		double dDot12 = DotProduct(nodeN21xN22, node12);
		double dDot21 = DotProduct(nodeN11xN12, node21);
		double dDot22 = DotProduct(nodeN11xN12, node22);
/*
...
*/
	// First edge is a line of constant latitude; second is a great circle arc
		...
		double dApexZ = sqrt(dAbsEqCross2 / dAbsCross2);
/*
		...
		//double dApexX = - dCrossX / dCrossZ;
		//double dApexY = - dCrossY / dCrossZ;

		double dApexExpectedLength = sqrt(1.0 - dApexZ * dApexZ);
		double dApexX = - dCrossX * dApexExpectedLength / sqrt(dAbsEqCross2);
		double dApexY = - dCrossY * dApexExpectedLength / sqrt(dAbsEqCross2);

		double dApexMag = dApexX * dApexX + dApexY * dApexY + dApexZ * dApexZ;
...
/*
	double dDotNbNb = DotProduct(nodeBegin, nodeBegin);
	double dDotNeNb = DotProduct(nodeEnd, nodeBegin);
...
/*
		...
		double dDotN0Nb = DotProduct(node0, nodeBegin);
		double dDotN2Nb = DotProduct(node2, nodeBegin);
...
*/
		...
		double dDotLR = DotProduct(nodeDirL, nodeDirR);
		double dDotLA = DotProduct(nodeDirL, nodeDirA);
		double dDotRA = DotProduct(nodeDirR, nodeDirA);
		...
		double dDotPlanar = DotProduct(nodeX, nodeDirR);
...
*/
		...
		if ((edge0.type == Edge::Type_GreatCircleArc) &&
			...
			double dDot = DotProduct(nodeNormal, nodeDir);
...
*/
		...
		if ((edgetype == Edge::Type_ConstantLatitude) &&
			...
			// Dot product
			double dDot = DotProduct(nodeDir, nodeNormal);
/*
			printf("F: %i %i\n",
