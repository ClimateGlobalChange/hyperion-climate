...
std::string LoadShpFile(
	...
	std::ifstream shpfile(
		strShpFilename.c_str(), std::ios::in | std::ios::binary);
	...
	while (iCurrentPosition < shphead.iFileLength) {
		...
		if (iShapeType != SHPPolygonType) {
			return std::string("ERROR: Record Polygon type expected");
		}
		...
		if (shppolyhead.nNumParts > 0x1000000) {
			return std::string("ERROR: Polygon NumParts exceeds sanity bound (%i)",
				shppolyhead.nNumParts);
		}
		if (shppolyhead.nNumPoints > 0x1000000) {
			return std::string("ERROR: Polygon NumPoints exceeds sanity bound (%i)",
