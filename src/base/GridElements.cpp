...
void Mesh::ConstructAdjacencyList(
	...
	for (; iter != edgemap.end(); iter++) {
		...
		if ((iter->second[0] != InvalidFace) &&
			...
			if ((iter->second[0] < 0) || (iter->second[0] >= faces.size())) {
				_EXCEPTION1("Face index (%i) out of range",
					iter->second[0]);
			}
			if ((iter->second[1] < 0) || (iter->second[1] >= faces.size())) {
				_EXCEPTION1("Face index (%i) out of range",
					iter->second[1]);
			}
...
void Mesh::ConstructReverseNodeArray() {
	...
	// Clear any existing data in the ReverseNodeArray
	revnodearray.resize(nodes.size());
	for (int i = 0; i < revnodearray.size(); i++) {
		revnodearray[i].clear();
	}

	// For each Face add the Face index to the array for all Nodes
	for (int i = 0; i < faces.size(); i++) {
...
double Mesh::CalculateFaceAreas(
...
double Mesh::CalculateFaceAreasFromOverlap(
...
void Mesh::Read(const std::string & strFile) {
	...
	if (iSCRIPFormat == 3) {
		...
		if (varGridCenterLat != NULL) {
			...
			if (attGridCenterLatUnits != NULL) {
				strLatUnits = attGridCenterLatUnits->as_string(0);
			}
		}
		if (varGridCenterLon != NULL) {
			if (varGridCenterLat->get_dim(0)->size() != nGridSize) {
		...
		// Get the grid area
		NcVar * varGridArea = ncFile.get_var("grid_area");

		if (varGridArea != NULL) {
			if (varGridArea->get_dim(0)->size() != nGridSize) {
				_EXCEPTIONT("Invalid dimension for \"grid_center_lat\"");
			}
			vecFaceArea.Allocate(nGridSize);
			varGridArea->set_cur((long)0);
			varGridArea->get(&(vecFaceArea[0]), nGridSize);
		}
	...
	// Input from a NetCDF Exodus file
		...
		// Load in node array
		{
			...
			for (int i = 0; i < nNodeCount; i++) {
				nodes[i].x = static_cast<double>(dNodeCoords[0][i]);
				nodes[i].y = static_cast<double>(dNodeCoords[1][i]);
				nodes[i].z = static_cast<double>(dNodeCoords[2][i]);
			}
...
void Mesh::Validate() const {
	...
	for (int i = 0; i < faces.size(); i++) {
		...
		for (int j = 0; j < nEdges; j++) {
			...
			double dDot = DotProduct(node1, nodeCross);
/*
			...
			if (fabs(nodeCross.x - nodeCross.fx.Todouble()) > ReferenceTolerance) {
			...
			if (fabs(nodeCross.y - nodeCross.fy.Todouble()) > ReferenceTolerance) {
				_EXCEPTIONT("FixedPoint mismatch (Y)");
			}
			if (fabs(nodeCross.z - nodeCross.fz.Todouble()) > ReferenceTolerance) {
...
bool IsPositivelyOrientedEdge(
	...
	const double Tolerance = ReferenceTolerance;
...
void GetLocalDirection(
	...
	if (edgetype == Edge::Type_GreatCircleArc) {
		...
		double dDotDirBegin = DotProduct(nodeDir, nodeRef);
		double dNormNodeBegin = DotProduct(nodeRef, nodeRef);
...
void GetLocalDirection(
	...
	if (edgetype == Edge::Type_GreatCircleArc) {
		...
		// Project onto surface of the sphere
		double dDotDirBegin   = DotProduct(nodeDir, nodeBegin);
		double dNormNodeBegin = DotProduct(nodeBegin, nodeBegin);
...
/*
		...
		double dDotN0N1 = DotProduct(node0, node1);
		double dDotN0N2 = DotProduct(node0, node2);
		double dDotN2N1 = DotProduct(node2, node1);

		double dDotN0N0 = DotProduct(node0, node0);
		double dDotN1N1 = DotProduct(node1, node1);
		double dDotN2N2 = DotProduct(node2, node2);

		double dDenom0 = dDotN0N0 * dDotN1N1 - dDotN0N1 * dDotN0N1;
		double dDenom2 = dDotN2N2 * dDotN1N1 - dDotN2N1 * dDotN2N1;
