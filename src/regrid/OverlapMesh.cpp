...
#ifdef CHECK_AREAS
		double dSourceFaceArea = meshSource.CalculateFaceArea(ixCurrentSourceFace);
#endif
...
*/
	...
	if (ixCurrentTargetFace == InvalidFace) {
		Announce(2, "\tNo overlapping face found");
		return;
	}

	Announce(2, "\tFirst overlap match %i", ixCurrentTargetFace);
/*
	// Verify starting Node is not on the Exterior
	if (aFindFaceStruct.loc == Face::NodeLocation_Exterior) {
		_EXCEPTIONT("Invalid Node location returned from FindFaceFromNode");
...
*/
		...
		if (nodevecOutput.size() == 0) {
			...
			Announce(2, "\tOverlap with Face %i", ixCurrentTargetFace);
			...
			double dArea = CalculateFaceArea(faceTemp, nodevecOutput);
...
void GenerateOverlapMesh_v2(
	...
	int iNextReportedPercent = 0;
	for (int i = 0; i < meshSource.faces.size(); i++) {
		double dPercent =
			100.0 * static_cast<double>(i)
			/ static_cast<double>(meshSource.faces.size());

		if (dPercent >= static_cast<double>(iNextReportedPercent)) {
			Announce("Source Face %i [%i%%]", i, iNextReportedPercent);
			iNextReportedPercent += 5;
		}
		...
		Announce(2, "\tNearest target face %i", iTargetFaceSeed);
