...
*/
		...
		// Multiply integration array and fit array
		DataArray2D<double> dComposedArray(
			nAdjFaces, meshThisElement.faces.size());
		...
		DataArray2D<double> dRedistributedArray(
			nAdjFaces, meshThisElement.faces.size());
...
/*
	...
	for (int ixSecond = 0; ixSecond < meshOutput.faces.size(); ixSecond++) {
		...
		DataArray1D<int> iPIV(nP * nP);

		DataArray1D<double> dWork(nP * nP);
	...
	DataArray1D<double> dTemp(nP * nP);
...
*/
	...
	for (int ixSecond = 0; ixSecond < meshOutput.faces.size(); ixSecond++) {
		...
		// Source areas
		DataArray1D<double> vecSourceArea(
			vecReverseFaceIx[ixSecond].size());
...
/*
	// Check consistency
	DataArray2D<double> dIntSums(
		meshOutput.faces.size(), nP * nP);
...
/*
	// Check conservation
	DataArray1D<double> dMassSums(
		meshInput.faces.size());
...
*/
/*
	DataArray1D<double> dOverlapMass(
		meshOutput.faces.size());
...
/*
		DataArray1D<double> dRowSum(nCoefficients);
...
*/
		...
		// Coefficients
		DataArray2D<double> dCoeff(
			nOverlapFaces * nPout * nPout, nPin * nPin);
